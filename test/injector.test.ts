...
Deno.test("injectAscii re-renders large diagram (>100 lines) idempotently without duplication", () => {
  ...
  const markerCount =
    result.filter((line) => line.includes("<!-- MAAR: big-diagram.mmd -->")).length;
...
Deno.test("injectAscii handles multiple diagrams in same file correctly", () => {
  ...
  const marker1Count =
    result1.filter((line) => line.includes("<!-- MAAR: diagram1.mmd -->")).length;
  const marker2Count =
    result1.filter((line) => line.includes("<!-- MAAR: diagram2.mmd -->")).length;
  ...
  const finalMarker1Count =
    result2.filter((line) => line.includes("<!-- MAAR: diagram1.mmd -->")).length;
  const finalMarker2Count =
    result2.filter((line) => line.includes("<!-- MAAR: diagram2.mmd -->")).length;
...
Deno.test("injectAscii early terminates at other mmd links and doesn't cross boundaries", () => {
  ...
  const linkIndex = result.findIndex((line) =>
    line.includes("[Second Diagram](second-diagram.mmd)")
  );
